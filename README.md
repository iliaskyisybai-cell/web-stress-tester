...
📖 How It Works
Unlike traditional synchronous scripts that send requests one by one, this tool utilizes Asynchronous I/O:

Event Loop: The script manages thousands of tasks simultaneously without waiting for each server response before sending the next one.

Semaphores: Implements concurrency control to prevent local system resource exhaustion (CPU/RAM).

TCP Connection Pooling: Efficiently reuses connections to maximize the pressure on the target server's network stack.
...
Rate Limiting: Configure Nginx/Apache to limit requests per second from a single IP (e.g., using limit_req in Nginx).

Web Application Firewall (WAF): Implementing solutions like Cloudflare to filter anomalous traffic patterns.

Fail2Ban: Automatically banning IP addresses that generate excessive 404 errors or rapid connection attempts.
